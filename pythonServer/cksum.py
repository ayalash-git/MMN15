...
class CheckSum(object):
    def __init__(self, file_name):
        self.logger = logging.getLogger("CheckSum")
    ...
    def memcrc(self, buffer):
        """
        crc calculation of given buffer
        :param buffer:  data we want to do checksum on it
        :return: checksum
        """
        n = len(buffer)
        i = c = s = 0
        for ch in buffer:
            tabindex = (s >> 24) ^ ch
            s = UNSIGNED((s << 8)) ^ self._crctab[tabindex]

        while n:
            c = n & 0o377
            n = n >> 8
            s = UNSIGNED(s << 8) ^ self._crctab[(s >> 24) ^ c]
        return UNSIGNED(~s)

    def getCheckSumFile(self):
        """
        calculate checksum CRC for file
        :return: check sum of file
        """
        try:
            buffer = open(self._file_name, 'rb').read()
            crc = self.memcrc(buffer)
            self.logger.info(f"CRC of {crc}\t{len(buffer)}\t File name{self._file_name}")
            return crc
        except IOError:
            print("Unable to open input file", self._file_name)
            exit(-1)
